...
# usage run.sh <redis_version> <source_box_name> [source_box_version]
# can pass auto as <redis_version> to install the current version in apt catalogue
...
if [ "$1" != "auto" ]; then
    export PKR_REDIS_VER=$1
else
    export PKR_REDIS_VER=""
fi
